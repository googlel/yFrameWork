package com.liy.today.recyclerview.adapter;
...
import com.liy.today.recyclerview.holder.RecyclerHeaderViewHolder;
...
//        addAnimation(holder);
    ...
    public void addHeaderViewList(List<View> v) {
        if (v != null && v.size() != 0) {
    ...
    public void addFooterViewList(List<View> v) {
        if (v != null && v.size() != 0) {
            for (int i = 0; i < v.size(); i++) {
                View view = v.get(i);
                if (null != view) {
